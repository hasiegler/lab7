...
The k_means function takes in a dataframe of numeric variables, so we first remove the _Species_ column of the iris dataset. For this example, we will use 3 clusters.
```{r}
iris2 <- iris %>% 
    select(-Species)
...
k$tot.withinss
```
The output of the k_means function returns the cluster assignment for each of the observations, as well as the total sum of squares.

__How well did the cluster assignment work?__
...
# with pca
set.seed(40)
results <- k_means(iris2, k = 3, pca = T)
...
The function hier_clust takes a dataframe of numeric variables and a number of clusters to stop at as arguments.
...
The cluster assignments are based on all the numeric variables in the iris dataset, however we can still see that the cluster assignments were somewhat accurate based on these two variables in the plot. The plot above only shows two of the variables used in the cluster assignments, however we still see that the observations were clustered into 3 groups.
